...
class WalletController extends Controller
{
    ...
    public function deposit(Request $request)
    {
        ...
        try {
            ...
            // Return back with error and old input
            return redirect()->back()
                ->withErrors(['amount' => $e->getMessage()], 'deposit')
                ->withInput();
        }
    ...
    public function transfer(Request $request)
    {
        Validator::make($request->all(), [
            ...
            'amount' => 'required|numeric|min:0.01',
            'notes' => 'nullable|string|max:255',
        ])->validateWithBag('transfer');

        $receiver = User::findOrFail($request->receiver_id);

        try {
            $this->walletService->transfer($request->user(), $receiver, (float)$request->amount, $request->notes ?? null);
        } catch (\Exception $e) {

            return redirect()->back()
                ->withErrors(['amount' => $e->getMessage()], 'transfer')
                ->withInput();
        }
